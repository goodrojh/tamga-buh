...
import { Calculator, ShoppingBag, Users, FileText, Rocket, XCircle, MapPin, Landmark, Receipt, MessageSquare, Globe, ArrowRight } from 'lucide-react'
import { oneTimeServices } from '@/data/pricing'
import { useLead } from '@/components/LeadModal'

const MAIN: { icon: typeof Calculator; title: string; desc: string[]; price: string; topic: string; href?: string }[] = [
  { icon: Calculator, title: 'Бухгалтерское сопровождение', desc: ['ИП и ООО на УСН, ПСН, УСН + ПСН, АвтоУСН, ОСНО, УСН + НДС.', 'Учёт, налоги, отчётность, ЕНС.'], price: 'от 5 000 ₽/мес', href: '#calculator', topic: 'Бухгалтерское сопровождение' },
  { icon: ShoppingBag, title: 'Учёт маркетплейсов', desc: ['Wildberries, Ozon, Яндекс Маркет и др.', 'Сверка отчётов, комиссии, возвраты, налог с правильной базы.'], price: 'включено в тариф', topic: 'Учёт маркетплейсов' },
  { icon: Users, title: 'Кадры и зарплата', desc: ['Приём и увольнение, оформление отпусков и больничных листов.', '6-НДФЛ, РСВ, ЕФС-1, персучёт.', 'Воинский учёт.'], price: 'от 1 000 ₽/сотр.', topic: 'Кадровый учёт и зарплата' },
  { icon: FileText, title: 'Нулевая отчётность', desc: ['Для ИП и ООО без деятельности — чтобы не заблокировали счёт и не начислили штрафы.'], price: 'от 2 500 ₽/кв', topic: 'Нулевая отчётность' },
  { icon: Rocket, title: 'Регистрация ИП и ООО', desc: ['В Калмыкии или в вашем регионе.', 'Подбираем коды ОКВЭД и режим налогообложения, готовим и подаём документы.'], price: 'ИП — бесплатно*', topic: 'Регистрация ИП / ООО' },
  ...
  { icon: MapPin, title: 'Юридический адрес в Калмыкии для ООО', desc: ['Реальный офис в Элисте, приём корреспонденции, гарантийное письмо.', 'Для регистрации со ставкой УСН 1 %.'], price: 'от 5 000 ₽/мес', topic: 'Юридический адрес в Калмыкии для ООО' },
  ...
  { icon: Receipt, title: '3-НДФЛ для физических лиц', desc: ['Вычеты за жильё, лечение, обучение.', 'Декларация 3-НДФЛ с продажи имущества, по ИИС, доходы из-за рубежа.'], price: 'от 1 500 ₽', topic: 'Декларация 3-НДФЛ' },
  { icon: Globe, title: 'Уведомление об открытии счёта за рубежом', desc: ['Для физических лиц: уведомление ФНС об открытии, закрытии или изменении реквизитов зарубежного счёта.', 'Отчёт о движении средств по счёту.'], price: 'от 1 500 ₽', topic: 'Уведомление об открытии счёта за рубежом' },
  { icon: MessageSquare, title: 'Консультация главного бухгалтера', desc: ['Разбор конкретной ситуации: запросы ФНС, выбор режима, переезд в Калмыкию, НДС на УСН.'], price: 'первые 15 мин — бесплатно', topic: 'Консультация главного бухгалтера' },
]
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-5">
          {MAIN.map(({ icon: Icon, title, desc, price, topic, href }, i) => (
            ...
            >
              ...
              <div className="text-[14px] text-gray-500 leading-relaxed mt-2 flex-1 space-y-1">
                {desc.map((line) => (
                  <p key={line}>{line}</p>
                ))}
              </div>
